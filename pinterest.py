...
class Pinterest:
    def __init__(self, query, max_count=100, detect_face=False, use_mtcnn=False):
        self.__query = query
    ...
    def __get_images(self):
        images = []
        for item in self.__raw_data["resource_response"]["data"]["results"]:
            images.append(item["images"]["474x"]["url"])
        return images
    ...
    def __download_images(self, bookmark=None):
        ...
        for image in images:
            ...
            print("Downloading image %s" % self.__count_downloaded)
...
if __name__ == "__main__":
    pin = Pinterest("Angry Man", max_count=100, detect_face=True)
    pin.merge_folder("angry man")
    pin.run()
