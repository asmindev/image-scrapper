...
class Pinterest:
    def __init__(
        ...
        self.__folder = self.__create_folder_init()
        self.__path = f"{self.__folder}/{query}"
    ...
    def __encode_params(self):
        params = {
            ...
            "data": [
                '{"options":{"article":"","appliedProductFilters":"---","query":"%s","scope":"pins","top_pin_id":"","filters":"","bookmarks":["%s"],"no_fetch_context_on_resource":true},"context":{}}'
                % (self.__query, self.__bookmark)
            ],
        }
                ...
                if self.__use_mtcnn:
                    ...
                    if len(faces) > 0:
                        for index, face in enumerate(faces):
                            ...
                            cv2.imwrite(
                                f"{self.__path}/{self.__query}_{self.__count_downloaded}_{index}.jpg",
                                face_img,
                            )
                else:
